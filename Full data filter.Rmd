...
query <- "SELECT * FROM media_space_2024"
data24 <- dbGetQuery(conn, query)
...
top25 <- data24 %>%
  ...
  filter(year(DATE) == 2024) %>%            # Filter for the year 2024
...
not_top25 <- data24 %>%
  filter(SOURCE_TYPE == "web")%>%
  filter(year(DATE) == 2024) %>%               # Ensure only year 2024 is considered
...
catoliq24 <- dta22_filter[has_word==T,]
...
write.xlsx(catoliq24, "C:/Users/lukas/Dropbox/HKS/Projekti/Dezinformacije/Data/Inicjalno citanje/catoliq24.xlsx")
