...
words_vector <- str_c("\\b(", str_c(pomocno$root, collapse = "|"), ")\\b")
...
for (i in 1:num_batches) {
  ...
  dt[start_idx:end_idx, `:=` (
    has_fraze = sapply(FULL_TEXT, check_matches, words_vector),
    matched_fraze = sapply(FULL_TEXT, function(x) paste(unlist(str_extract_all(x, words_vector)), collapse=", "))
  )]
...
fraze <- dt[has_fraze==T,]
#write.xlsx(fraze, "C:/Users/lukas/Dropbox/HKS/Projekti/Dezinformacije/Data/Inicjalno citanje/fraze.xlsx")
#write.csv2(fraze, "./fraze.csv")
```


```{r echo=F, eval=T, message=F , warning= FALSE}
fraze <- read.xlsx("C:/Users/lukas/Dropbox/HKS/Projekti/Dezinformacije/Data/Inicjalno citanje/fraze.xlsx") #%>% select(-c(1,2))
```
